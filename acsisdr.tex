...
is then used by the baseline  fitting routine, \textsc{mfittrend} , to detertmine
...
emission features. 
The Starlink \textsc{cupid} \citep{2007ASPC..376..425B} application contains 
...
The Starlink KAPPA task \textsc{findclump} is used to do the clump
...
band). This applies to many Galactic targets. Smoothing is biases
...
different techniques are required for handling the automated baseline
subtraction.

\textit{\color{red} Actually, the recipe itself implies that it's more or less the
same as the standard narrow-band recipe. *RPT* I think the only
difference is that the moment routines makes maps for each line}
...
\textit{\color{red}  *RPT* I don't think this recipe
was ever properly  implemented: the idea here was to do
a baseline fit but to restore the DC component of the fit, not to
just omit the fit}
