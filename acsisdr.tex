...
the sky is measured by different receptors and to help minimize
...
area on the sky tends to be measured by a single receptor and this can
...
at different angles, and with the telescope moved to different offsets
 to allow different receptors to be combined. Unless the source of
...
\citep[][\ascl{1310.007}]{SUN258} is used for this. This application
...
To maximize overall aperture efficiency, receptors in HARP are spaced
...
\caption{Examples of low-frequency interference by receptor.
 It shows time-averaged (clipped mean) spectra for each receptor in
 which the third and ninth (labelled \#10) receptor from the bottom
 exhibit strong global non-linear baselines.  Some other receptors
...
signal.  These comprise a threshold to remove spikes and mask the
...
automatic mode (\mbox{\S \ref{sec:mfittrend}}) to determine the baseline
regions and hence the remaining emission.  \mfittrend\ on its own can
have difficulty separating broadline emission from baseline.  One
...
{\color{red} Now discuss the pipeline implementation. Before and after
  flatfielding examples: Fig.~\ref{fig:flatfield:results}.}
...
\caption{A flat-field comparison dislayed in an integrated intensity
  ...
  receptor-to-receptor performance.  The right panel shows the same
  ...
  exhibit weaker receptor differences than this example.}
...
Active galaxies often have broad lines of several hundreds of km/s so this recipe is tuned
