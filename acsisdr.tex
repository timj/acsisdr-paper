...
(***RPT: the figure is incorrect: we do not make a cube for individual observation
...
Finally, the reference pixel coordinates is shifted by up to one pixel
...
\footnote{If the positions do not form a regular grid, an option is
...
The resampling can use either of the three schemes described in the
previous section to determine how bad pixels are propagated from input
...
(***RPT: remove the noise spikes at the end of the subbands or also
show the resulting combined spectrum ***)
