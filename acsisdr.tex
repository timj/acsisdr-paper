...
is then used by the baseline  fitting routine, \textsc{mfittrend} from
Starlink KAPPA \citep{SUN95,2008ASPC..394..650C}, to determine
...
(see \S \ref{sec:moment}) is then be used with masked data set to
...
in place for the JCMT Science Archive.
...
comprehensive Gauss-Hermite multi-profile fitting task \textsc{Fit1d}  was
added to the SMURF package \citep{SUN258} that produces a cube with the fitted profiles as well
...
\textit{\color{red} At one point Per was mentioning that Kriging
  \citep[e.g.][]{1990Cressie} would be a useful
